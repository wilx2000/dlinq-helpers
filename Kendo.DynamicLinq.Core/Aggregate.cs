	...
	public class Aggregator
	{
		...
		public MethodInfo MethodInfo(Type type)
		{
			...
			switch (Aggregate)
			{
				case "max":
				case "min":
					return GetMethod(StringHelper.ToTitleCase(Aggregate), MinMaxFunc().GetMethodInfo(), 2).MakeGenericMethod(type, proptype);
				case "average":
				case "sum":
					return GetMethod(StringHelper.ToTitleCase(Aggregate), 
